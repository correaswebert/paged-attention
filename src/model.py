...
class PagedAttentionModel(torch.nn.Module):
    ...
    def forward_layer(
        ...
        v = layer.self_attn.v_proj(normed).view(
            batch_size, 1, self.num_kv_heads, self.head_dim
        )
        ...
        x += layer.mlp(layer.post_attention_layernorm(x))
        return x

    @torch.no_grad()
    def forward_batch(
        self,
        batch_tokens: list[TokenId],
        batch_positions: list[int],
        batch_block_tables: list[BlockTable],
        kv_manager: KVCacheManager,
    ) -> list[TokenId]:
        # [batch_size, 1, hidden_dim]
        x = torch.tensor(batch_tokens, device=self.device).unsqueeze(1)
        x: Tensor = self.embed(x)

        # Pad block tables to a square tensor
        max_blocks = max(len(t) for t in batch_block_tables)
        padded_tables = [t + [0] * (max_blocks - len(t)) for t in batch_block_tables]
        block_table_tensor = torch.tensor(
            padded_tables, dtype=torch.int32, device=self.device
        )

        # number of tokens already in cache per request
        cache_seqlens = torch.tensor(
            batch_positions, dtype=torch.int32, device=self.device
        )

        # absolute position of the token being processed now
        ...
        next_tokens: list[TokenId] = torch.argmax(logits[:, -1, :], dim=-1).tolist()
