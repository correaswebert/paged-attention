...
__global__ void flashattention_kernel(const half* __restrict__ q,
                                      const half* __restrict__ k,
                                      const half* __restrict__ v,
                                      half* __restrict__ out, int seq_len,
    ...
    half* o_bh = out + (bh_idx * seq_len) * head_dim;
    ...
    for (int idx = threadIdx.x; idx < q_tile_size; idx += blockDim.x) {
        ...
        q_tile[local_row_idx][d_idx] =
            (global_row_idx < seq_len)
                ? __half2float(q_bh[global_row_idx * head_dim + d_idx])
                : 0.0f;
    ...
    for (int j = 0; j < T_c; j++) {
        ...
        for (int idx = threadIdx.x; idx < kv_tile_size; idx += blockDim.x) {
            ...
            k_tile[local_row_idx][d_idx] =
                (global_row_idx < seq_len)
                    ? __half2float(k_bh[global_row_idx * head_dim + d_idx])
                    : 0.0f;
        }
        ...
        for (int idx = threadIdx.x; idx < kv_tile_size; idx += blockDim.x) {
            ...
            v_tile[local_row_idx][d_idx] =
                (global_row_idx < seq_len)
                    ? __half2float(v_bh[global_row_idx * head_dim + d_idx])
                    : 0.0f;
        }
    ...
    if (q_row_idx < seq_len) {
        ...
        for (int d = 0; d < head_dim; d++) {
            o_bh[(q_row_idx * head_dim) + d] =
                __float2half(l_inv * o_tile[q_tile_row_idx][d]);
        }
    }
}
...
torch::Tensor custom_flash_attention(torch::Tensor q, torch::Tensor k,
    ...
    flashattention_kernel<B_r, B_c><<<grid_size, block_size>>>(
        reinterpret_cast<const half*>(q_bh.const_data_ptr<at::Half>()),
        reinterpret_cast<const half*>(k_bh.const_data_ptr<at::Half>()),
        reinterpret_cast<const half*>(v_bh.const_data_ptr<at::Half>()),
        reinterpret_cast<half*>(out_bh.mutable_data_ptr<at::Half>()), seq_len,
        head_dim, causal);
